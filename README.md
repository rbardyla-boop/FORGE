...
W4's credentialless broker bridge is `BRIDGE_PASS`, while its real-binary live gate remains separate. The no-secret official v0.147.0 binary preflight is recorded as `BLOCKED_EXTERNAL: LIVE_API_KEY_UNAVAILABLE`; no real OpenAI request, ChatGPT credential, or billable request was made.
